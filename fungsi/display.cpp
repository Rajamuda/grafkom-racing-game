void printTxt(int x,int y,int z, char *string, int info=0){
...
void display(){
    if(!countDown){
        ...
        printTxt(widthWindow/2-70,heightWindow-70,10, txt3);
//        system("cls");
//        std::cout<<"printf: "<< duration <<'\n';
    }
    char txt1[] = "Speed: ";
    char txtBuff[15];
    snprintf(txtBuff,sizeof(txtBuff),"%.2f km/h",(mobil[0].getSpeed(1)));
    strcat(txt1, txtBuff);
...
//    printf("LAP: %d", maxLap);
    ...
    if(mobil[0].getLap() == 0){
        int lap = 1;
        snprintf(txtBuff2,sizeof(txtBuff2),"%d of %d",lap, maxLap);
    }else{
        snprintf(txtBuff2,sizeof(txtBuff2),"%d of %d",(mobil[0].getLap()), maxLap);
    }
    strcat(txt2,txtBuff2);
    printTxt(widthWindow-210,60,10,txt2);
